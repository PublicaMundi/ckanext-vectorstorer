...
def vectorstorer_upload( geoserver_cont,cont,data):
	...
	update_resource_format(context,entity)
...
def publish_layer(geoserver_context,table_name):
    ...
    url=geoserver_url+"/rest/workspaces/"+geoserver_workspace+"/datastores/ckan_datastore_default/featuretypes"
...
def update_resource_format(context,resource):
    api_key=context['apikey'].encode('utf8')
    resource['format']='Shapefile'
    data_string = urllib.quote(json.dumps(resource))
    request = urllib2.Request('http://localhost:5000/api/action/resource_update')
